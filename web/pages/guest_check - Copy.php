...
if ($age >= 60 && !in_array(2, $post_data['vulnerable'])) {
    $post_data['vulnerable'][] = 2;
        ...
        $citizen_id = 0;
        if (!$old_data) {
            $sql = "INSERT INTO citizens (id_card_hash, id_card_enc, id_card_last4, prefix, firstname, lastname, gender, birthdate, address_id, addr_number, phone_enc, medical_info, notes, photo_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if ($decision == 'update') {
                // เพิ่ม id_card_last4 เข้าไปใน UPDATE ด้วยเพื่อความแม่นยำ
                $sql = "UPDATE citizens SET prefix=?, firstname=?, lastname=?, gender=?, birthdate=?, address_id=?, addr_number=?, phone_enc=?, medical_info=?, notes=?, photo_path=?, id_card_last4=? WHERE id=?";
        ...
        <div class="col-lg-10">
            ...
            <div class="card shadow-sm compare-card">
                ...
                <form action="guest_check.php" method="POST">
                    <?php renderHiddenInputs($post_data); ?>
