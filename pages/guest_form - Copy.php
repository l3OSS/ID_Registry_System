...
$(document).ready(function() {
    // Address Autocomplete (jquery.Thailand.js)
    if($.Thailand) {
        $.Thailand({
            ...
            onSelect: function(data) {
                // Background lookup for internal address ID
                fetch(`api/address_id.php?district=${data.district}&amphoe=${data.amphoe}&province=${data.province}`)
                    .then(res => res.json())
                    .then(res => { if(res.status === 'success') $('#address_id').val(res.address_id); });
            }
        });
    }

    // Reset Address ID if manual typing occurs (Ensures clean data)
    $('#addr_tambon, #addr_amphoe, #addr_province').on('input', function(e) {
        if (e.isTrusted) $('#address_id').val('');
    });
...
async function readSmartCard() {
    ...
    try {
        ...
        // ตรวจสอบเพศ (รองรับทั้งแบบตัวอักษรภาษาอังกฤษ ภาษาไทย และตัวเลข)
        // ดึงค่าเพศและลบช่องว่างออก (ถ้ามี) เพื่อการเช็คที่แม่นยำ
        let rawGender = data.Gender ? data.Gender.toString().trim() : "";
        let detectedGender = "";

        // ตรวจสอบเงื่อนไขเพศชาย: รองรับ "1", "Male", "ชาย"
        if (rawGender == "1" || rawGender.toLowerCase() === "male" || rawGender === "ชาย") {
            detectedGender = "Male";
        } 
        // ตรวจสอบเงื่อนไขเพศหญิง: รองรับ "2", "Female", "หญิง"
        else if (rawGender == "2" || rawGender.toLowerCase() === "female" || rawGender === "หญิง") {
            detectedGender = "Female";
        }

        // ส่งค่าที่ประมวลผลแล้วไปยังฟิลด์ Select
        setVal('gender', detectedGender);

        // Log ดูค่าเพื่อการตรวจสอบ (Debug)
        console.log("Raw Gender Data:", data.Gender, "-> Detected as:", detectedGender);
        
        // Address Mapping
        setVal('addr_number', data.HouseNo + (data.Moo ? ` หมู่ ${data.Moo.replace(/\D/g,'')}` : ''));
        setVal('addr_tambon', data.Tambon.replace(/ตำบล|ต\./g, '').trim());
        setVal('addr_amphoe', data.Amphoe.replace(/อำเภอ|อ\./g, '').trim());
        setVal('addr_province', data.Province.replace(/จังหวัด|จ\./g, '').trim());

        // Background Match for Address ID
        fetch(`api/address_id.php?district_code=${data.CitizenID.substring(0,6)}`)
            .then(res => res.json())
            .then(res => { if(res.status === 'success') setVal('address_id', res.address_id); });
