...
try {
    ...
    if ($existing_citizen_id > 0) {
        ...
        if ($decision === 'update') {
            $sql = "UPDATE citizens SET id_card_enc=:enc, id_card_last4=:last4, prefix=:prefix, firstname=:fname, lastname=:lname, 
                    gender=:gender, birthdate=:dob, address_id=:aid, addr_number=:anum, phone_enc=:phone, medical_info=:med, notes=:notes, is_consent=:consent, updated_at=NOW() WHERE id=:cid";
    ...
    // --- CASE B: New Record or Edit from existing ID ---
        ...
        if ($id > 0) {
            ...
            $sql = "INSERT INTO citizens (id_card_hash, id_card_enc, id_card_last4, prefix, firstname, lastname, gender, birthdate, address_id, addr_number, phone_enc, medical_info, notes, is_consent) 
                    VALUES (:hash, :enc, :last4, :prefix, :fname, :lname, :gender, :dob, :aid, :anum, :phone, :med, :notes, :consent)";
